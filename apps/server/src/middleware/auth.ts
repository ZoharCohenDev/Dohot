...
export async function requireAuth(req: Request, res: Response, next: NextFunction) {
  ...
  if (!authHeader?.startsWith('Bearer ')) {
    logger.warn({ method: req.method, path: req.path }, 'Auth failed: missing or malformed Authorization header');
  ...
  if (!user) {
    logger.warn({ method: req.method, path: req.path }, 'Auth failed: invalid or expired token');
