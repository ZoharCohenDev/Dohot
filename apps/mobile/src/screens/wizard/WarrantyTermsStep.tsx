...
export function WarrantyTermsStep({ colors = lightColors, onNext, onBack }: WarrantyTermsStepProps) {
  ...
  // Conditions list
  const initConditions = wizard.state.warrantyConditions.length > 0
        ...
        >
          ...
          <View>
              ...
              {PRESET_DURATIONS.map((d) => (
                ...
                >
                  <Text style={[
                    ...
                    { color: selectedDuration === d ? colors.bg : colors.ink1, fontFamily: fonts.sans },
                  ]}>
                    {d}
                  </Text>
                </Pressable>
            ...
            {selectedDuration === 'אחר' && (
              <View style={[styles.customDurationInput, { borderColor: colors.line, backgroundColor: colors.bgElev }]}>
                <TextInput
          ...
          <View>
            ...
            {/* Existing conditions */}
            <View style={[styles.conditionsList, { borderColor: colors.line, backgroundColor: colors.bgElev }]}>
              {conditions.map((cond, idx) => (
                <View
                  key={idx}
                  style={[
                    styles.conditionRow,
                    idx < conditions.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.line },
                  ]}
                >
                  {editingIndex === idx ? (
                    /* Edit mode */
                    <View style={styles.conditionEditRow}>
                      <TextInput
                        style={[styles.conditionEditInput, { color: colors.ink1, fontFamily: fonts.sans, borderColor: colors.line, backgroundColor: colors.bg }]}
                        value={editText}
                    ...
                    <View style={styles.conditionDisplayRow}>
                      <View style={[styles.conditionNumBadge, { backgroundColor: colors.bgSunken }]}>
                        <Text style={[styles.conditionNum, { color: colors.ink3, fontFamily: fonts.sans }]}>
                          {idx + 1}
                        </Text>
                      </View>
                      <Text style={[styles.conditionText, { color: colors.ink1, fontFamily: fonts.sans }]} numberOfLines={3}>
                        {cond}
                      </Text>
              ...
              {/* Add new condition */}
              <View style={[styles.addConditionRow, { borderTopWidth: conditions.length > 0 ? 1 : 0, borderTopColor: colors.line }]}>
                <TextInput
                ...
                <Pressable
                  ...
                  style={[styles.addConditionBtn, { backgroundColor: newCondition.trim() ? colors.ink1 : colors.bgSunken }]}
                  hitSlop={4}
                >
            ...
            <View style={styles.previewRow}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.previewText, { color: colors.ink2, fontFamily: fonts.sans }]}>
                  ...
                  {workDescription ? `  •  ${workDescription.slice(0, 50)}${workDescription.length > 50 ? '…' : ''}` : ''}
                </Text>
                <Text style={[styles.previewText, { color: colors.ink3, fontFamily: fonts.sans, marginTop: 2 }]}>
