...
export function QuoteItemsStep({ colors = lightColors, onNext, onBack }: QuoteItemsStepProps) {
  ...
  const [customValidity, setCustomValidity] = useState(
    isPresetValidity || !initValidity ? '' : initValidity
  );
  ...
  const handleDelete = (key: string) => {
    Alert.alert('מחק פריט', 'האם למחוק פריט זה?', [
      { text: 'ביטול', style: 'cancel' },
      {
        text: 'מחק',
        style: 'destructive',
        onPress: () => setItems((prev) => prev.filter((i) => i.key !== key)),
      },
    ]);
  };
  ...
  return (
    ...
    >
      ...
      <View
        style={[
          styles.formCard,
          { backgroundColor: colors.bgElev, borderColor: isEditing ? colors.accent : colors.line },
        ]}
      >
        {isEditing && (
          ...
          <View
            style={[
              styles.inputRow,
              {
                ...
                borderColor: errors.title ? colors.danger : colors.lineStrong,
              },
            ]}
          >
            <TextInput
        ...
        <View style={styles.fieldGroup}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor: colors.bg,
                borderColor: errors.price ? colors.danger : colors.lineStrong,
              },
            ]}
          >
            <Text style={[styles.currencyPrefix, { color: colors.ink3, fontFamily: fonts.sans }]}>
              ₪
            </Text>
            <TextInput
              style={[styles.textInput, { color: colors.ink1, fontFamily: fonts.sans }]}
              ...
              <View
                ...
                style={[
                  styles.itemCard,
                  {
                    ...
                    borderColor: editingKey === item.key ? colors.accent : colors.line,
                  },
                ]}
              >
                {/* Number + title + price */}
                <View style={styles.itemTopRow}>
                  <View style={[styles.itemNumBadge, { backgroundColor: colors.ink1 }]}>
                    <Text
                      style={[styles.itemNumText, { color: colors.bg, fontFamily: fonts.sans }]}
                    >
                      {idx + 1}
                    </Text>
                  </View>
                  <Text
                    style={[styles.itemTitle, { color: colors.ink1, fontFamily: fonts.sans }]}
                    numberOfLines={1}
                  >
                    {item.title}
                  </Text>
                ...
                <View style={[styles.itemActions, { borderTopColor: colors.line }]}>
                  ...
                  >
                    ...
                    <Text
                      style={[styles.actionBtnText, { color: colors.ink2, fontFamily: fonts.sans }]}
                    >
                      ערוך
                    </Text>
                  </Pressable>
      ...
      {items.length > 0 && (
        <View
          style={[styles.totalsCard, { backgroundColor: colors.bgElev, borderColor: colors.line }]}
        >
          <View style={styles.totalRow}>
            <Text style={[styles.totalLabel, { color: colors.ink3, fontFamily: fonts.sans }]}>
              סכום לפני מע״מ
            </Text>
          ...
          <View style={[styles.totalRow, styles.totalRowMid]}>
            ...
            <Text style={[styles.totalValue, { color: colors.ink3, fontFamily: fonts.sans }]}>
              {formatILS(vat)}
            </Text>
          </View>
          <View style={[styles.totalRow, styles.totalRowFinal, { borderTopColor: colors.line }]}>
            <Text style={[styles.totalLabelBig, { color: colors.ink1, fontFamily: fonts.sans }]}>
              סה״כ לתשלום
            </Text>
            <Text style={[styles.totalValueBig, { color: colors.accent, fontFamily: fonts.sans }]}>
              {formatILS(total)}
            </Text>
          </View>
        </View>
      ...
      {/* ── Quote validity ── */}
      <View
        style={[styles.validityWrap, { backgroundColor: colors.bgElev, borderColor: colors.line }]}
      >
        <Text
          style={[
            styles.fieldLabel,
            { color: colors.ink2, fontFamily: fonts.sans, marginBottom: 10 },
          ]}
        >
          תוקף ההצעה
        </Text>
            ...
            >
              <Text
                style={[
                  ...
                  {
                    color: selectedValidity === v ? colors.bg : colors.ink1,
        ...
        {selectedValidity === 'אחר' && (
          <View
            style={[
              styles.inputRow,
              { marginTop: 8, backgroundColor: colors.bg, borderColor: colors.lineStrong },
            ]}
          >
            <TextInput
              style={[styles.textInput, { color: colors.ink1, fontFamily: fonts.sans }]}
      ...
      {/* ── Notes ── */}
      <View
        style={[styles.notesWrap, { backgroundColor: colors.bgElev, borderColor: colors.line }]}
      >
        <Text
          style={[
            styles.fieldLabel,
            { color: colors.ink2, fontFamily: fonts.sans, marginBottom: 8 },
          ]}
        >
...
const styles = StyleSheet.create({
  ...
  validityRow: { flexDirection: 'row-reverse', flexWrap: 'wrap', gap: 8 },
  validityPill: {
    paddingHorizontal: 14,
    paddingVertical: 8,
    borderRadius: 10,
